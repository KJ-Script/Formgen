...
function choiceGenerator() {
    ...
    document.getElementById('confirm-choice-button').addEventListener('click', () => {
        questions.push({
            type: "select",
...
function renderQuestions() {
    ...
    questions.forEach(question => {
        switch(question.type) {
            case "select": {
                const optionsString = question.options.map(option => `<option value="${option}}">${option.toUpperCase()}</option>`).join("");
